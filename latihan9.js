...
function groupByCategory(products) {
     return products.reduce((groups, product) => {
         ...
         if (!groups[key]) groups[key] = []; //gatau cara baca ini
         groups[key].push(product); 
         return groups; 
        }, {}); 
    }
    console.log(groupByCategory(products))
const gabungan = groupByCategory(products);

const summary = [];

for (const category in gabungan) {
    summary.push({
        kategori: category,
        jumlahProduk: gabungan[category].length
    });
}
